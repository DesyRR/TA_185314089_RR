...
public class MessageEventGenerator implements EventQueue {
	...
	// Interval between messages (min, max) -> waktu interval min dan max dalam pembuatan pesan
        
        // perbedaan waktu untuk generate pesan
        // condition = msgInterval[0] == msgInterval[1],
        // if condition true maka interval di set 0 
        // if condition false ambil random number dari msgInterval[1] dan msgInterval[0]
            int timeDiff = msgInterval[0] == msgInterval[1] ? 0 : 
			rng.nextInt(msgInterval[1] - msgInterval[0]);
                // mengambil nilai random, kemudian dikali dengan interval pembuatan pesan max - min
               // int u = (int) (Math.random() * (msgInterval[1] - msgInterval[0]) + msgInterval[0]);
